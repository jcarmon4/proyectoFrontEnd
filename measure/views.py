...
def measure(request):
    ...
    if 'value' in request.GET:
        ...
        if value:
            ...
            response = requests.post('http://pi1-eafit-alejocram.azurewebsites.net/measures/', args)
            # Convierte la respuesta en JSON
            measure_json = response.json()

    # Realiza una petición GET al Web Services
    response = requests.get('http://pi1-eafit-alejocram.azurewebsites.net/measures/')
    # Convierte la respuesta en JSON
